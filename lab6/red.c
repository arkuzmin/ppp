...
int my_random(int rank)
{
	srand(rank + time(NULL));
	return rand();
}

void print_result(const char* method, int node, int max, double time)
{
	printf("%s: Maximum value %d found on %d (%f seconds)\n", method, max, node, time);
}
